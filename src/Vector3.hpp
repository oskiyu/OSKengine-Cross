...
namespace OSK {
	...
	public:
		...
		constexpr Vector3_t(const T& x, const T& y, const T& z) : x(x), y(y), z(z) {

		}

		/// @brief Crea una instancia del Vector2 en la que X, Y y Z tienen en mismo valor.
		/// @param value Valor para X, Y y Z.
		explicit constexpr Vector3_t(const T& value) : x(value), y(value), z(value) {

		}

		/// @brief Crea un vector equivalente a @p vec.
		explicit constexpr Vector3_t(const glm::vec3& vec) : x(vec.x), y(vec.y), z(vec.z) {
		...
		constexpr Vector3_t operator+(const Vector3_t& vec) const {
			return Vector3_t(x + vec.x, y + vec.y, z + vec.z);
		}

		/// @param vec Otro vector.
		/// @return Vector actual sumándole @p vec.
		constexpr Vector3_t& operator+=(const Vector3_t& vec) {
			x += vec.x;
			y += vec.y;
			z += vec.z;
			return *this;
		...
		constexpr Vector3_t operator*(const T& value) const {
			return Vector3_t(x * value, y * value, z * value);
		}

		/// @brief Multiplica todas las coordenadas por el valor.
		/// @return Self (X * value; Y * value; Z * value).
		constexpr Vector3_t& operator*=(const T& value) {
			x *= value;
			y *= value;
			z *= value;
			return *this;
		...
		constexpr T GetDistanceTo2(const Vector3_t& vec) const {
			T dx = x - vec.x;
			T dy = y - vec.y;
			T dz = z - vec.z;
		...
		constexpr T Dot(const Vector3_t& vec) const {
			return x * vec.x + y * vec.y + z * vec.z;
		}

		/// @return Producto vectorial entre este vector y @p vec.
		constexpr Vector3_t Cross(const Vector3_t& vec) const {
		...
		/// Rango [0, 180].
		constexpr T GetAngle(const Vector3_t& vec) const {
		...
		constexpr Vector3_t GetNormalized() const {
			T length = GetLenght();
		...
		constexpr glm::vec3 ToGlm() const {
		...
		template <typename P> constexpr inline Vector3_t<P> ToVec3() const {
			return Vector3_t<P>(x, y, z);
		}

		
		/// @return Vector con float.
		constexpr inline Vector3_t<float_t> ToVector3f() const {
			return ToVec3<float>();
		}

		/// @return Vector con double.
		constexpr inline Vector3_t<double_t> ToVector3d() const {
			return ToVec3<double_t>();
		}

		/// @return Vector con int.
		constexpr inline Vector3_t<int32_t> ToVector3i() const {
			return ToVec3<int32_t>();
		}

		/// @return Vector con unsigned int.
		constexpr inline Vector3_t<uint32_t> ToVector3ui() const {
