	...
	m_type(type)
{
	if (materialName) {
		...
		for (UIndex16 i = 0; i < Engine::GetRenderer()->GetSwapchainImagesCount(); i++) {
			m_materialInstance.Insert(m_material->CreateInstance());
		}
	}
}
...
void IRenderPass::_Execute(ICommandList* cmdList, RenderGraph& renderGraph) {
	...
	if (m_type == RenderPassType::GRAPHICS) {
		...
		Vector4ui windowRec = {
			0,
			0,
			m_colorTargets[0].targetImage->GetSize2D().x,
			m_colorTargets[0].targetImage->GetSize2D().y
		};
...
void IRenderPass::BindResources(ICommandList* cmdList, RenderGraph& renderGraph) {
	...
	for (USize16 i = 0; i < Engine::GetRenderer()->GetSwapchainImagesCount(); i++) {		
		...
		for (const auto& dependency : m_bufferDependencies) {
			...
			if (dependency.usage == RgBufferUsage::STORAGE_BUFFER && m_material) {
			...
			else if (dependency.usage == RgBufferUsage::UNIFORM_BUFFER && m_material) {
				m_materialInstance[i]
					->GetSlot(dependency.shaderBinding.slotName)
					->SetUniformBuffer(
						dependency.shaderBinding.bindingName,
		...
		for (const auto& dependency : m_imageDependencies) {
			...
			// Storage image.
			if (dependency.usage == GpuExclusiveImageUsage::COMPUTE_OR_RT_TARGET && m_material) {
				m_materialInstance[i]
					->GetSlot(dependency.shaderBinding.slotName)
					->SetStorageImage(
						dependency.shaderBinding.bindingName,
			...
			// Sampler.
			else if (dependency.usage == GpuExclusiveImageUsage::SAMPLED && m_material) {
				m_materialInstance[i]
					->GetSlot(dependency.shaderBinding.slotName)
					->SetGpuImage(
						dependency.shaderBinding.bindingName,
		...
		if (m_material && m_materialInstance[i].HasValue()) {
...
void IRenderPass::BindTargetImages(RenderGraph& renderGraph) {
	auto i = Engine::GetRenderer()->GetCurrentFrameIndex();
...
void IRenderPass::RebindDependencies() {
	m_needsResourceBinding = true;
}

RenderGraph* IRenderPass::GetRenderGraph() {
	return m_rg;
}
