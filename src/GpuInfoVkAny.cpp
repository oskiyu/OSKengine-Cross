...
bool GpuInfoVkAny::IsRtCompatible() const {
...
bool GpuInfoVkAny::IsCompatibleWithMeshShaders() const {
	const auto availableExtensions = GpuVk::GetAvailableExtensions(physicalDevice);
...
bool GpuInfoVkAny::SupportsMemoryBudgetQuery() const {
	const auto availableExtensions = GpuVk::GetAvailableExtensions(physicalDevice);

	return
		GpuVk::IsExtensionPresent(VK_EXT_MEMORY_BUDGET_EXTENSION_NAME, availableExtensions);
}
...
GpuInfoVkAny GpuInfoVkAny::Get(VkPhysicalDevice gpu, VkSurfaceKHR surface) {
	GpuInfoVkAny info{};
