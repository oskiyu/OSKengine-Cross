	...
	: ISdfRenderer2D(ecs), m_ecs(ecs), m_memoryAllocator(allocator) 
{
	...
	const auto resourcesInFlightCount = Engine::GetRenderer()->GetSwapchainImagesCount();

	m_globalInformationBuffers.Resize(resourcesInFlightCount);
	m_globalInformationInstances.Resize(resourcesInFlightCount);
	m_drawCallsBuffers.Resize(resourcesInFlightCount);
	m_drawCallsBuffersInstances.Resize(resourcesInFlightCount);

	for (UIndex32 i = 0; i < m_globalInformationInstances.GetSize(); i++) {
